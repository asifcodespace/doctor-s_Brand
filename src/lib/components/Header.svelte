...
<header
  class="fixed top-0 left-0 right-0 z-100 h-[80px] flex items-center px-16 md:px-6 gap-8 transition-all duration-300 bg-primary"
  class:scrolled={false}
>
  <!-- Logo -->
  <a
    href="/"
    class="font-display text-2xl text-cream tracking-wider mr-auto whitespace-nowrap opacity-90"
  >
    {config.doctor.name}
  </a>
  ...
  <!-- Mobile Toggle -->
  <button
    class="hidden flex-col gap-[5px] p-2 bg-none border-none cursor-pointer sm:flex"
    onclick={toggleMenu}
    aria-label="Toggle menu"
  >
    <span
      class="block w-[22px] h-[2px] bg-mist rounded-[2px] transition-all duration-200"
  ...
  {#if isMenuOpen}
    <div
      class="fixed top-[80px] left-0 right-0 bg-primary flex flex-col px-12 sm:px-6 py-8 border-t border-white/5 animate-slide-down shadow-xl"
    >
      <a
        href="#services"
        class="text-mist no-underline text-sm py-3 border-b border-white/5 tracking-widest"
      ...
      <a
        href="#contact"
        class="text-accent no-underline text-sm py-4 font-bold tracking-widest"
